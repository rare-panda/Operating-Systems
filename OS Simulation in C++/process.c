...
sem_t pmutex, psyncmutex, pipmutex;
...
{ PCB[pid]->exeStatus = CPU.exeStatus;
  ...
  insert_termIO (pid, PCB[pid]->sockfd, str, exitProgIO);
...
void initialize_process_manager ()
{
  ...
  init_idle_process ();
  sem_init (&pmutex, 0, 1);
  sem_init (&psyncmutex, 0, 0);
  sem_init (&pipmutex, 0, 1);
}
...
{ int pid, ret, i;
  ...
  insert_termIO (pid, sockfd, str, exitProgIO);
  return (-1);
}
...
{ int pid, intime;
  ...
  { // *** ADD CODE to execute the ready process (with pid# = pid)
	...
	context_in(pid);
	CPU.exeStatus = eRun;
	int cyclesBefore = CPU.numCycles;
	event = add_timer(cpuQuantum,CPU.Pid,actTQinterrupt,oneTimeTimer);
	cpu_execution();
	PCB[pid]->timeUsed += CPU.numCycles - cyclesBefore;
	context_out(pid);
