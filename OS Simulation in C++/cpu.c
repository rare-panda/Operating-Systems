  ...
  { case OPload:
    ...
    case OPprint:
      ...
      if(mret == mError)
      {
        CPU.exeStatus = eError;
	if(cpuDebug)
	  fprintf(bugF,"print page lo error undi\n");
	break;
      }
      else if(mret == mPFault)
      {
        CPU.exeStatus = ePFault;
	if(cpuDebug)
	  fprintf(bugF,"print page lo pFault exc undi\n");
	break;
      }
      else
      {
        char *str = (char *) malloc (80);
        sprintf(str, "pid=%d, M[%d]=%2.f",CPU.Pid, CPU.IRoperand, CPU.MBR);
        insert_termIO(CPU.Pid, CPU.sockfd, str, regularIO);
        CPU.exeStatus = eWait;
      }
      break;
    case OPsleep:
      event = add_timer(CPU.MBR, CPU.Pid, actReadyInterrupt, oneTimeTimer);
      CPU.exeStatus = eWait;
      break;
    case OPexit:
      CPU.exeStatus = eEnd;
